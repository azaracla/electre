...
def concordance(data = data, weight = weight, order = order):
    """ Retourne la concordance à partir du tableau des critères, des poids de chaque critère et l'ordre de classement"""
...
def PP(data = data, weight = weight, order = order):
    """ Calcule la matrice des P+/P- : inutile dans cette implémentation finalement. """
...
def threshold_c(data = concordance(), c0 = c0, c1 = c1, c2 = c2):
    """ Classe les concordances en faibles (f), moyen (m) et fort (F) en fonction des seuils c0, c1, c2"""
...
def discordance(data = data, weight = weight, order = order):
    """ Retourne la discordance à partir du tableau des critères, des poids de chaque critère et l'ordre de classement"""
...
def threshold_d(data = discordance(), d1 = d1, d2 = d2):
    """ Classe les discordances en faibles (f), moyen (m) et fort (F) en fonction des seuils d1 et d2"""
...
def graphe_fort(C = threshold_c(), D = threshold_d()):
    """ Retourne un tableau représentant le graphe fort : un 1 symbolise un arc entre i et j"""
...
def graphe_faible(C = threshold_c(), D = threshold_d(), graphe_fort = graphe_fort()):
    """ Retourne un tableau représentant le graphe faible : un 1 symbolise un arc entre i et j"""
...
def edges(graphe):
    """ Renvoie les arcs sous forme de liste"""
...
def compute_graphs(grph_F = graphe_fort(), grph_f = graphe_faible()):
    """ Calcule les graphes fort et faible et les retourne sous forme networkx """
...
def preprocessor(G):
    """ Renvoie un graphe dictionnaire à partir d'un graphe networkx """
...
def postprocessor(G):
    """ Renvoie un graphe networkx à partir d'un graphe dictionnaire """
...
def dfs(graph, start, end):
    """ Algorithme de recherche de graphe en profondeur (Depth First Search) """
...
def cycle(GF, Gf):
    """ Elimine les circuits dans le graphe networkx G et retourne un dictionnaire"""
...
def compute_red_graphs(grph_F = graphe_fort(), grph_f = graphe_faible()):
    """ Retourne les graphes réduits sous forme de dictionnaire """
...
def plot_red(GFr = GFr, Gfr = Gfr):
    """ Affiche les graphes réduits avec la bibliothèque networkx """
...
def sort_actions(GFr = GFr, Gfr = Gfr):
    ...
    while GFr and i < 100:
        i+=1
        B = [] # ensemble des actions qui ne sont surclassées fortement par aucune autre action
        
        surclassesF = [] #ensembles des noeuds surclassés du graphe fort.
...
def robustesse_poids():
    """ Calcule la robustesse de la méthode par rapport aux poids choisis en les faisant varier un par un """
...
def robustesse_seuils_c(): 
    """ Calcule la robustesse de la méthode par rapport aux seuils choisis en les faisant varier un par un """ 
    order = compute_custom()
...
def aleas(data,i):
    """ Retourne les données avec un aléas en pourcentage """
    data_alea=data.copy()
    n, m = len(data), len(data[0])
...
def robustesse_aleas(data = data):
    """ Calcule la robustesse de la méthode par rapport aux données d'entrée""" 
